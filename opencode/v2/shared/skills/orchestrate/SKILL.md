...
- **Medium/coupled:** make a short plan and implement directly. Delegate only substantial external research (@Researcher) or a fresh-eyes review that would genuinely change the outcome.
...
3. Keep tightly coupled or overlapping-file work in one session. Dispatch only isolated or genuinely independent phases to @Worker; send @Researcher bounded queries for substantial external facts and request concise cited findings.
