...
external TUI component reacts to plugin-local Solid signals. The counter should
advance continuously without hiding the sidebar, changing routes, or remounting
the slot.
...
With CLI, plugin, and theme build `0.0.0-next-17028`, the counter remains at
`0s` until the sidebar is hidden and shown. The test uses a component-local
`createSignal`, creates its timer in `onMount`, cleans it up with `onCleanup`,
and returns the child component from `context.ui.slot`. This isolates the
failure from OpenCode data caches, `context.storage`, and the production plugin
logic.
