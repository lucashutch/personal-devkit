...
1. Select the `Advisor` role with model `astra` and effort `medium`. If the role or model is unavailable, report the limitation; never silently substitute or change configuration.
