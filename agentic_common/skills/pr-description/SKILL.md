...
4. Include `Validation` only for useful commands and results. State breaking changes and migration steps under `Breaking changes`; add applicable issue links. No boilerplate test plans, checklists, placeholders, or other optional sections. A one-change PR may need only the summary.
