...
- `~/.bashrc` is not managed by the linker, so `dotfiles/bashrc.d/` snippets only load if it sources `~/.config/bashrc.d/*.sh`. When shell helpers such as `oc` are missing, check for that loader first and add the snippet from `docs/dotfiles.md` after confirming with the user, keeping a backup of the original file.
