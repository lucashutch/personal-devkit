...
The OpenCode group is not purely declarative. Beyond the links it also installs
the plugin dependencies in `opencode/` and configures the test profile's service
on port `4099`. It does not guarantee that the installed SDK matches the
installed CLI build; see [upgrading OpenCode](opencode.md#upgrading-opencode).
